---
name: arenaforge-agent-tools
description: "Deterministic helper layer for emulating ArenaForge tools in Codex or Claude Code when native TreeView, TreeAddNode, TreeSetMeta, RunExecutor, GitMergeBranch, or report tooling is unavailable. Use for local state management, eval score capture, executor prompt generation, merge checks, and skill forward tests."
---

# ArenaForge Agent Tools

Use this skill when the host does not provide native ArenaForge tools. The bundled
script stores state in the same style as open-source ArenaForge:

```text
<cwd>/.arenaforge/runtime/sessions/<run_name>/.coordinator/idea_tree.json
<cwd>/.arenaforge/runtime/sessions/<run_name>/.coordinator/idea_tree.md
```

## Script

`scripts/research_state.py` is stdlib-only.
...
ArenaForge repository, initialize a short run, record metadata, add one idea,
