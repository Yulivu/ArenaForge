---
name: arenaforge-agent-merge-eval
description: "Merge and evaluation discipline for ArenaForge. Use for TreeSetMeta metadata, B_dev/B_test separation, eval command templates, score parsing, GitMergeBranch behavior, protected paths, required outputs, metric_direction, trunk/test score updates, medal detection, and final evaluation before stopping."
---

# ArenaForge Merge And Eval
...
4. If `test_baseline_score` is missing and a baseline test run is feasible,
   record it.
5. Hand off to `arenaforge-agent-resume-report`.
...
state that no separate B_test was used, run `research_state.py check`, and hand
off to report generation.

## Manual Emulation

If native `GitMergeBranch` is unavailable, use `arenaforge-agent-tools`:

```bash
python <tools>/research_state.py eval --cwd <project> --run-name <run> \
  --split dev --cmd "<eval_cmd>" --set-meta baseline

python <tools>/research_state.py meta --cwd <project> --run-name <run> \
  --set "trunk_branch=<trunk_branch>"

python <tools>/research_state.py merge --cwd <project> --run-name <run> \
