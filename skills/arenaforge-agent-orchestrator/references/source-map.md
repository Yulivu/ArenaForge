# ArenaForge Source Map

Use this file when auditing the skill suite against the current ArenaForge source
tree.
...
- `src/cli/commands/run.py`: `arenaforge research-run`, intake, preflight,
...
- `src/coordinator/prompts.py`: coordinator identity and full ArenaForge
  cycle protocol.
...
- The open-source branch uses the `arenaforge` CLI, intake planning, session
