---
name: arenaforge-research-agent
description: "Public entrypoint for the ArenaForge skill suite. Use when a user wants to run an ArenaForge-style autonomous research or optimization workflow from a natural-language goal, including initial clarification of objective, target project, data, metric, evaluation, permissions, budget, run mode, and then automatic bootstrapping into arenaforge-agent-orchestrator and phase skills."
---

# ArenaForge Research Agent

Use this as the single user-facing entrypoint. The user should be able to say
`$arenaforge-research-agent` plus a plain-language goal, similar to using `arenaforge`,
without knowing the internal phase skills.

This skill performs ArenaForge-style intake and clarification, then hands control to
...
3. Decide whether the request is clear enough to start. If not, ask concise
   ArenaForge-style clarification questions before optimizing.
4. Once the contract is clear, load `arenaforge-agent-orchestrator` and continue
...
- For training logs or progress logs, avoid raw `cat`, raw `grep`, or broad
  `tail`. If `arenaforge-agent-tools` is available, use `research_state.py parse-log`;
  otherwise normalize carriage returns with `tr '\r' '\n'` and inspect only the
  metric lines needed for the contract.
- Defer deep code reading to `arenaforge-agent-setup-intake`, `arenaforge-agent-executor`,
...
Then load `arenaforge-agent-orchestrator` and pass this contract as the run
...
- In smoke mode, load `arenaforge-agent-tools` when native ArenaForge tools are absent,
  use cached metrics or `research_state.py parse-log`, generate
...
1. Load `arenaforge-agent-orchestrator`.
2. Ensure `arenaforge-agent-setup-intake` receives the contract.
   - If the target is not eval-ready (no runnable eval, no dev/test split, or a
     dirty/absent git repo), the `arenaforge-agent-setup-intake` phase scaffolds the
...
3. Let the orchestrator load phase skills as needed:
   - `arenaforge-agent-coordinator`
   - `arenaforge-agent-ideate`
   - `arenaforge-agent-executor`
   - `arenaforge-agent-merge-eval`
   - `arenaforge-agent-search`
   - `arenaforge-agent-plugins-hitl-budget`
   - `arenaforge-agent-resume-report`
   - `arenaforge-agent-tools`
4. Keep the user-facing behavior at the ArenaForge level. Do not expose internal
...
```text
$arenaforge-research-agent optimize this repo for leaderboard score overnight
```

or:

```text
$arenaforge-research-agent try a simplified smoke run on this autoresearch repo
```
...
- create or select `.arenaforge/runtime/sessions/<run_name>/`;
- initialize durable Idea Tree state;
- run the ArenaForge loop through orchestrator and phase skills;
