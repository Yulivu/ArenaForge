...
When emulating manually, do the same sequence. Use `arenaforge-agent-tools` for
...
2. Generate the prompt from the main session while substituting the executor
   worktree as `{cwd}`:
   ```bash
   python <tools>/research_state.py prompt-executor --cwd <project> --run-name <run> \
     --node-id <id> --workdir <worktree> \
     --output <project>/.arenaforge/runtime/sessions/<run>/experiments/<id>/executor_prompt.md
   ```
...
6. Record outcome:
   ```bash
   python <tools>/research_state.py record --cwd <project> --run-name <run> \
