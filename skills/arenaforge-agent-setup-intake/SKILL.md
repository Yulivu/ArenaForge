---
name: arenaforge-agent-setup-intake
description: "Setup, intake, preflight, and launch-contract phase for open-source ArenaForge runs. Use when confirming a target project, metric, baseline, dev/test split, config/plugin settings, branch guard, session directory, or when translating a user goal into the precise contract consumed by the coordinator."
---

# ArenaForge Setup And Intake

Use this before the coordinator starts. The output is a concrete research
contract plus a clean workspace/session ready for the ArenaForge cycle.
...
7. Initialize or select `.arenaforge/runtime/sessions/<run_name>/` and hand the contract
   to the coordinator.
8. If real merges are allowed, define a non-protected `trunk_branch` such as
   `arenaforge/trunk/<run_name>`. Treat `main`/`master` as the base branch, not the
   merge target.
...
- Fallback when the MCP server is absent:
  `arenaforge benchmark scaffold <dir> [--style zoo] [--git-init]`.
...
- `research_config.yaml` — machine-readable, auto-detected by `arenaforge`:
  ...
  Follow `examples/arbor_config.example.yaml`.

These are written during setup, before handing the contract to the coordinator.

## Preflight Checks

Mirror `arenaforge research-run` preflight:
...
```text
<project>/.arenaforge/runtime/sessions/<run_name>/
  REPORT.md
...
When no native ArenaForge runtime exists, initialize the same layout with:

```bash
TOOLS="<skill-dir>/arenaforge-agent-tools/scripts/research_state.py"
python "$TOOLS" init \
...
`arenaforge-agent-plugins-hitl-budget` before INIT because the plugin pre-fills
...
When using `research_state.py meta`, quote any `--set` value that contains
...
python "$TOOLS" meta --cwd <project> --run-name <run_name> \
  ...
  --set "trunk_branch=arenaforge/trunk/<run_name>"
```

In smoke mode, if a cached baseline log exists, parse only metric lines with
`research_state.py parse-log` or an equivalent parser that normalizes carriage
...
arenaforge setup
arenaforge doctor
arenaforge research-run "<contract>" --yes --yes-cwd <project> --config <config> --max-cycles 3
arenaforge report <project>/.arenaforge/runtime/sessions/<run_name>
```

Skill-only smoke tests should use `arenaforge-agent-tools` and a copied project
