"""Session export helpers for ArenaForge Research Runtime runs.
...
def resolve_session_dir(session: Path, cwd: Path | None = None) -> Path:
    """Resolve a session path or name under ``<cwd>/.arenaforge/runtime/sessions``.

    Mirrors the report resolution behavior, but raises a
    ...
    if candidate.is_absolute():
        ...
        candidates.extend([
            candidate,
            cwd / candidate,
            cwd / CONFIG_DIR_NAME / "runtime" / "sessions" / str(session),
        ])
        # When invoked from a linked git worktree, keyless sessions are anchored
        # at the main working tree (see session_ops._stable_runtime_base), so look
        ...
        try:
            from .mcp.session_ops import _stable_runtime_base

            stable = _stable_runtime_base(cwd)
            if stable != cwd:
                candidates.append(
                    stable / CONFIG_DIR_NAME / "runtime" / "sessions" / str(session)
                )
...
def _write_jsonl_export(data: dict[str, Any], output_path: Path) -> None:
    lines = [
        json.dumps({
            "type": "arenaforge.research_runtime.session_export",
...
def _render_html(data: dict[str, Any]) -> str:
    ...
    summary = data["summary"]
    title = summary.get("task") or summary.get("session_name") or "ArenaForge Research Runtime session"
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>{_esc_text(title)} - ArenaForge Research Runtime Export</title>
  <style>
{_EXPORT_CSS}
  </style>
</head>
<body>
  <div id="app">
    <aside id="sidebar">
      <div class="brand">ArenaForge Research Runtime Export</div>
