...
# 562). This keeps lightweight, keyless consumers — e.g. the `arenaforge-runtime mcp` server,
# `arenaforge-runtime export`, `arenaforge-runtime report` — from loading any LLM code just by touching the
# `arenaforge-runtime.core` package. `from arenaforge.research_runtime.core import Agent` etc. still work unchanged.
_LAZY_EXPORTS = {
    "Agent": ".agent",
    "AgentConfig": ".config",
