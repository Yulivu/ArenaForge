...
Claude Pro/Max subscribers can drive ArenaForge Research Runtime with their subscription instead of a
...
:class:`arenaforge-runtime.core.llm.claude_oauth.ClaudeOAuthProvider`.

Tokens live in ``~/.arenaforge/runtime/oauth/anthropic.json`` (chmod 600). Using a Claude
...
def refresh(tokens: AnthropicTokens) -> AnthropicTokens:
    ...
    if not tokens.refresh_token:
        raise OAuthError("no refresh token available; run `arenaforge-runtime login claude` again")
    data = _post_token({
...
def get_valid_tokens() -> AnthropicTokens:
    ...
    tokens = load_tokens()
    if tokens is None:
        raise OAuthError("not logged in to Claude — run `arenaforge-runtime login claude`")
...
def login(
    *, open_browser: bool = True, code_input: Callable[[], str] | None = None
    ...
    print("\nOpen this URL to authorize ArenaForge Research Runtime with your Claude account:")
    print(f"\n  {url}\n")
    if not opened:
