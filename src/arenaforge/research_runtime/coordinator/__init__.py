"""Coordinator — arenaforge-runtime-guided research orchestrator with Idea Tree.
...
def __getattr__(name: str) -> Any:
    ...
    keyless ``arenaforge-runtime mcp`` server, which only needs ``IdeaTree`` — from loading
    any LLM code. ``from arenaforge.research_runtime.coordinator import CoordinatorOrchestrator`` still
