...
The push-based WebUI (:mod:`arenaforge-runtime.webui.server`) normally renders a live
...
mutates session state through the ``arenaforge-runtime mcp`` tools, which persist to
``.arenaforge/runtime/sessions/<run>/``. This module reads that on-disk state and produces the
*same* snapshot dict shape the browser already understands
(:func:`arenaforge-runtime.webui.snapshot.empty_state_dict`), so the existing page renders
progress with no changes.

The Idea Tree JSON has no timing, so a sibling *activity sidecar*
(``.coordinator/activity.json``, written by :mod:`arenaforge-runtime.mcp.session_timing`)
...
def build_session_snapshot(session_dir: Path, run_name: str | None = None) -> dict[str, Any]:
    ...
    state["agents"] = agents
    state["arenaforge"] = _load_arenaforge_artifacts(session_dir)
    return state


def _load_arenaforge_artifacts(session_dir: Path) -> dict[str, Any]:
    """Expose ArenaForge artifacts in the existing ArenaForge Research Runtime snapshot contract."""

    link = _load_json(session_dir / "arenaforge_run.json")
    run_dir = Path(str(link.get("run_dir") or "")) if link.get("run_dir") else None
    if run_dir is None or not run_dir.is_dir():
        return empty_state_dict()["arenaforge"]
    from .snapshot import _read_arenaforge_artifacts
