...
class WebUIServer:
    ...
    def __init__(self, run_state: Any, bus: Any, *, port: int,
                 ...
                 product_api: Any | None = None) -> None:
        ...
        self._bootstrap_snapshot: dict[str, Any] | None = None
    ...
    def start(self) -> bool:
        ...
        if self.snapshot_fn is not None:
            # File-backed campaign projections can scan a sizeable artifact tree.
            # Warm the first snapshot before accepting browser connections so the
            # user never sees the empty shell while the first request is parsing.
            try:
                self._bootstrap_snapshot = self.snapshot_fn()
            except Exception:
                log.debug("initial WebUI snapshot warmup failed", exc_info=True)
        try:
    ...
    def current_snapshot(self) -> dict[str, Any]:
        """Build the JSON state used by both the bootstrap endpoint and SSE."""
        if self._bootstrap_snapshot is not None:
            state = self._bootstrap_snapshot
    ...
    def snapshot_frame(self) -> str:
        ...
        # in-memory RunState.
        return json.dumps({"kind": "snapshot", "state": self.current_snapshot()})
...
class _Handler(BaseHTTPRequestHandler):
    server_version = "arenaforge-webui/1.0"
    ...
    def do_GET(self) -> None:  # noqa: N802 (stdlib name)
        ...
        if path == "/":
            ...
            self._serve_json(200, self._webui.current_snapshot())
        elif path == "/events":
            self._serve_events()
        elif path == "/healthz":
            self._serve_text(200, "ok")
        elif path.startswith("/api/") and path.endswith("/export/download") and self._webui.product_api is not None:
            self._serve_export_download(path)
        elif path.startswith("/api/") and self._webui.product_api is not None:
            self._serve_product_api("GET", path)
        else:
    ...
    def do_POST(self) -> None:  # noqa: N802 (stdlib name)
        webui = self._webui
        path = urlparse(self.path).path
        if path.startswith("/api/") and webui.product_api is not None:
            self._serve_product_api("POST", path)
            return
        if path != "/input":
            self._serve_text(404, "not found")
            return
        ...
        supplied = self.headers.get("X-ArenaForge-Token") or _query_param(self.path, "t")
    ...
    def do_PATCH(self) -> None:  # noqa: N802 (stdlib name)
        path = urlparse(self.path).path
        if path.startswith("/api/") and self._webui.product_api is not None:
            self._serve_product_api("PATCH", path)
            return
        self._serve_text(404, "not found")

    def _serve_product_api(self, method: str, path: str) -> None:
        api = self._webui.product_api
        parts = [unquote(part) for part in path[len("/api/"):].split("/") if part]
        payload: Any = None
        if method in {"POST", "PATCH"}:
            ...
            try:
                ...
                return
        try:
            status, result = api.handle(method, parts, payload)
            ...
            log.exception("product API request failed")
            self._serve_json(500, {"ok": False, "error": "internal server error"})
            return
        self._serve_json(status, result)

    def _serve_export_download(self, path: str) -> None:
        """Generate and stream a campaign reproducibility archive.

        Product API calls normally return JSON. Export is different: returning
        the generated path leaves the browser with nothing it can actually
        download, so this dedicated endpoint streams the ZIP as an attachment.
        """

        parts = [unquote(part) for part in path[len("/api/"):].split("/") if part]
        if len(parts) != 4 or parts[0] != "campaigns" or parts[2:] != ["export", "download"]:
            self._serve_text(404, "not found")
            return
        try:
            result = self._webui.product_api.service.export(parts[1])
            archive = Path(str(result["export"])).resolve()
            if not archive.is_file() or archive.suffix.lower() != ".zip":
                raise FileNotFoundError("export archive is unavailable")
            body = archive.read_bytes()
        except (FileNotFoundError, KeyError, ValueError) as exc:
            self._serve_json(404, {"ok": False, "error": str(exc)})
            return
        except OSError:
            log.exception("failed to read campaign export")
            self._serve_json(500, {"ok": False, "error": "failed to read export archive"})
            return
