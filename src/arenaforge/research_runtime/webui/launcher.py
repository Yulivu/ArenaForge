...
def start_session_webui(
    ...
    *,
    ...
    ``arenaforge-runtime mcp`` tools. Walks up to ``scan`` ports from ``preferred`` to find a
    ...
    try:
        pointer_data = __import__("json").loads(pointer.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        pointer_data = {}
    if pointer_data.get("campaign"):
        from ...campaign_api import CampaignAPI
        from ...campaign_controller import CampaignController
        from ...campaign_service import CampaignService

        campaign_root = session_dir.parent
        product_api = CampaignAPI(
            CampaignService(campaign_root.parent),
            CampaignController(),
        )
