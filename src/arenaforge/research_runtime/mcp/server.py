"""The ``arenaforge-runtime mcp`` server — exposes :mod:`arenaforge-runtime.mcp.session_ops` over MCP.
...
only manages ArenaForge Research Runtime's durable state and research guards.
...
installed. Install it with ``pip install arenaforge-runtime-agent[mcp]``.

Each tool takes an explicit ``run_name`` (the ArenaForge Research Runtime session) and an optional
...
_MISSING_SDK_HINT = (
    ...
    "(or `uv pip install 'arenaforge-runtime-agent[mcp]'`)."
)
...
def build_server() -> Any:
    ...
    server = FastMCP("arenaforge-runtime")
    ...
    def scaffold_benchmark(
        ...
        """Create the ArenaForge Research Runtime-ready reference folder in cwd.
...
def run() -> None:
    """Entry point for ``arenaforge-runtime mcp`` — run the server over stdio.
