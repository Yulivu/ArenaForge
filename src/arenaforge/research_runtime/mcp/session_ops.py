"""Deterministic, keyless ArenaForge Research Runtime session operations (the engine behind ``arenaforge-runtime mcp``).
...
operations give it ArenaForge Research Runtime's durable state model and research discipline:

* **Idea Tree** state via the *real* :class:`arenaforge-runtime.coordinator.idea_tree.IdeaTree`
  (so the on-disk ``idea_tree.json`` is byte-compatible with the native runtime,
  the skill-suite fallback, and ``arenaforge-runtime export``).
...
Session layout (matches the native runtime and the skill suite)::
    ...
    <cwd>/.arenaforge/runtime/sessions/<run_name>/
...
(``skills/arenaforge-runtime-agent-tools/scripts/arbor_state.py``, which uses ``os.getuid()``),
...
def _safe_run_name(run_name: str) -> str:
    ...
    ``<cwd>/.arenaforge/runtime/sessions/``. We map anything outside ``[A-Za-z0-9_.-]`` to
...
def _stable_runtime_base(cwd: str | Path) -> Path:
    """Anchor the ArenaForge runtime root at the *main* working tree, not a linked one.
    ...
    directory, so a session created under ``<worktree>/.arenaforge/runtime`` is invisible from
    ...
    shared git common dir) and anchor ``.arenaforge/runtime`` there.
...
def session_dir(cwd: str | Path, run_name: str) -> Path:
    """Return ``<base>/.arenaforge/runtime/sessions/<run_name>`` (the per-run session root).

    ``base`` is normally *cwd*, but is redirected to the main working tree when
    *cwd* is a linked git worktree (see :func:`_stable_runtime_base`) so the
    session — and the WebUI watching it — survive branch/worktree switches.
    """
    return _stable_runtime_base(cwd) / ".arenaforge/runtime" / "sessions" / _safe_run_name(run_name)
...
def _user_tag() -> str:
    ...
    try:
        ...
        name = "arenaforge"
    return re.sub(r"[^A-Za-z0-9_.-]+", "-", name) or "arenaforge"
...
def scaffold_benchmark(
    ...
    *,
    ...
    """Create the ArenaForge Research Runtime reference folder under *cwd*; optionally git-init it.
    ...
    if git_init and not (target / ".git").exists():
        ...
        rc, _ = git(
            target,
            "-c", "user.email=arenaforge-runtime@localhost", "-c", "user.name=ArenaForge Research Runtime",
...
def _worktree_base(kind: str) -> Path:
    ...
    base = Path(tempfile.gettempdir()) / f"arenaforge-runtime-{kind}-{_user_tag()}"
