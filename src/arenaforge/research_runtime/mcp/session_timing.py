"""Lightweight timing/activity sidecar for keyless ArenaForge Research Runtime sessions.

The keyless MCP path (:mod:`arenaforge-runtime.mcp.session_ops`) persists the Idea Tree, but
the :class:`~arenaforge-runtime.coordinator.idea_tree.Node` model intentionally carries **no
...
event log. The WebUI's :mod:`arenaforge-runtime.webui.session_source` reads it to fill
