"""``arenaforge-runtime.zoo.scaffold`` — create the reference benchmark folder structure.
...
the ``zoo`` style, a natural-language ``README.md`` (the task description ArenaForge Research Runtime reads at
...
_SOLUTION = '''"""solution.py — the editable baseline (ArenaForge Research Runtime's edit surface).

Replace this with a working baseline. It must be correct first; ArenaForge Research Runtime then
...
#   bash eval.sh dev    # ArenaForge Research Runtime iterates here
...
def _readme(name: str, direction: str, edit: list[str], eval_entrypoint: str,
            split_kind: str) -> str:
    """Render a natural-language README — the task description ArenaForge Research Runtime reads at intake."""
    ...
    return (
        ...
        "## What ArenaForge Research Runtime may edit\n"
...
def _next_steps(style: str) -> list[str]:
    ...
    if style == "zoo":
        steps.append("Complete `README.md` (the task for ArenaForge Research Runtime) and `PROVENANCE.md` (for humans).")
        steps.append("Run `arenaforge-runtime benchmark verify <dir>` until it exits 0.")
    return steps
...
def scaffold_benchmark(
    ...
    *,
    ...
    """Scaffold the ArenaForge Research Runtime reference folder under *target*. See module docstring."""
