...
The collection *spine* (:mod:`arenaforge-runtime.zoo.collect`) is deterministic — it acquires
...
It reuses the core :class:`~arenaforge-runtime.core.agent.Agent` runtime (the same one the executor
...
dataset) into a runnable ArenaForge Research Runtime benchmark in the current directory. You write the *measurement
...
Produce, in the current directory:
  ...
  * the editable baseline (e.g. `solution.py`) — the simplest correct reference, the thing
    ArenaForge Research Runtime will later optimize. Do NOT optimize it.
  * `README.md` — plain language: the task, the metric (and whether higher/lower is better),
    which file(s) ArenaForge Research Runtime may edit, and how dev/test differ.
  * `PROVENANCE.md` — for humans: Source, Setup & environment, Baseline, Contamination
    assessment, Caveats.

The "baseline" is the *starting point ArenaForge Research Runtime will optimize*, NOT a SOTA method. It may come
...
You are DONE when the four artifacts exist, `arenaforge-runtime benchmark verify .` would pass, and the
...
def _parse_score(text: str) -> float | None:
    # Mirrors arenaforge-runtime.mcp.session_ops.parse_score for the documented `score: <float>` line,
    # kept local so arenaforge-runtime.zoo stays dependency-light.
    import re
...
PapersWithCode, and leaderboards. For each candidate, judge:
  ...
  * does the task have headroom to optimize (an artifact ArenaForge Research Runtime can edit), not just measure
    a frozen model?
...
The "baseline" is the *starting point ArenaForge Research Runtime will optimize*, NOT the repo's headline method.
A repo that proposes a method usually also ships simpler baselines (direct generation, naive
RAG, an earlier system); for ArenaForge Research Runtime those simpler baselines are the harvestable ones, because
...
def real_agent_runner(
    *,
    ...
    console-backed :class:`~arenaforge-runtime.zoo.ask_tool.ConsoleAskUserTool` so it can put a genuinely
    ...
    :mod:`arenaforge-runtime.zoo` stays light.
    """
    async def _run(*, cwd: Path, system_prompt: str, task: str, max_turns: int) -> str:
        from arenaforge.research_runtime.core import Agent, AgentConfig, create_provider
        from arenaforge.research_runtime.core.tools import get_all_tools
        ...
        if with_search:
            from arenaforge.research_runtime.coordinator.config import SearchConfig
            from arenaforge.research_runtime.core.tools.web.factory import (
