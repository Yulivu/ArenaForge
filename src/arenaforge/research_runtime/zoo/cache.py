...
git-ignored cache *outside* the ArenaForge Research Runtime checkout — reused across projects, never
...
The cache root is ``~/.arenaforge/runtime/cache/benchmarks`` by default, overridable via the
...
def cache_root() -> Path:
    """The global benchmark cache root (env override, else ``~/.arenaforge/runtime/cache/benchmarks``)."""
    override = os.environ.get(CACHE_ENV)
    if override:
        return Path(override).expanduser().resolve()
    return (Path.home() / ".arenaforge/runtime" / "cache" / "benchmarks").resolve()
