"""Pack discovery for the ``arenaforge-runtime-zoo`` benchmark format.

A *benchmark* is a directory under ``arenaforge-runtime-zoo/`` holding a self-contained task:
a natural-language ``README.md`` (what the task is, which score to optimize, what
ArenaForge Research Runtime may edit — read by ArenaForge Research Runtime at intake), a ``PROVENANCE.md`` card for humans, a
...
verification work by convention — see :mod:`arenaforge-runtime.zoo.verify` and ``docs/zoo.md``.
"""
...
class PackSummary:
    """Lightweight index entry for ``arenaforge-runtime benchmark list`` / the zoo README."""
