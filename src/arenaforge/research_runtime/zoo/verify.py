"""``arenaforge-runtime benchmark verify`` — a light structural lint for a benchmark folder.

The zoo format is documentation-first and natural-language: the README describes the
task for ArenaForge Research Runtime in plain prose (no machine manifest), and PROVENANCE is the human card.
...
@dataclass
class VerifyResult:
    """One check outcome. Same shape as ``arenaforge-runtime.cli.preflight.CheckResult`` but defined
    here to keep ``arenaforge-runtime.zoo`` free of any dependency on ``arenaforge-runtime.cli``."""
...
def _check_readme(pack_dir: Path) -> VerifyResult:
    """The README (ArenaForge Research Runtime's task description) exists and has some content."""
    ...
    if len(readme.read_text(encoding="utf-8").strip()) < 20:
        return VerifyResult("readme", "fail", "README.md is essentially empty",
                            "describe the task, the metric, and what ArenaForge Research Runtime may edit")
