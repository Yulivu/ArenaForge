...
def _domain(tree_meta: dict[str, Any], session_dir: Path) -> str:
    ...
    if not d:
        # session lives at <project>/.arenaforge/runtime/sessions/<run>; recover the project name.
        parts = session_dir.resolve().parts
        d = parts[-4] if len(parts) >= 4 and parts[-3:-1] == ("sessions",) else ""
        if not d and ".arenaforge/runtime" in parts:
            d = parts[parts.index(".arenaforge/runtime") - 1]
