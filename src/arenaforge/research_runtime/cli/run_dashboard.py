...
class RunDashboard:
    ...
    def _start_app(self) -> bool:
        ...
        self._app_thread = threading.Thread(target=self._run_app, daemon=True,
                                            name="arenaforge-runtime-dashboard-app")
    ...
    def _start_paint_loop(self) -> None:
        ...
        self._paint_thread = threading.Thread(target=run, daemon=True,
                                              name="arenaforge-runtime-dashboard-paint")
    ...
    def _start_stdin_reader(self) -> None:
        ...
        self._stdin_thread = threading.Thread(target=run, daemon=True,
                                              name="arenaforge-runtime-dashboard-stdin")
    ...
    def _start_prompt_reader(self) -> None:
        ...
        self._stdin_thread = threading.Thread(target=run, daemon=True,
                                              name="arenaforge-runtime-dashboard-prompt")
        self._stdin_thread.start()
        self._input_active = True
        self.state.mark_dirty()
    ...
    def _start_line_stdin_reader(self) -> None:
        ...
        self._stdin_thread = threading.Thread(target=run, daemon=True,
                                              name="arenaforge-runtime-dashboard-stdin-line")
        self._stdin_thread.start()
        self._input_active = True
        self.state.mark_dirty()
    ...
    def _render_header(self) -> Panel:
        ...
        title = Text()
        title.append("arenaforge-runtime", style="bold yellow")
    ...
    def _render_ideas(self) -> Padding:
        ...
        # (e.g. the literal arenaforge-runtime cycle root, or out-of-order events) get
...
# ``~/.arenaforge/runtime/crashes/<timestamp>.log`` by default) so we can find out
...
def _dump_crash(where: str, exc: BaseException) -> str | None:
    ...
    try:
        ...
        if env_path:
            ...
            base = os.path.expanduser("~/.arenaforge/runtime/crashes")
            os.makedirs(base, exist_ok=True)
            stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            path = os.path.join(base, f"{stamp}.log")
        body = (
            f"# arenaforge-runtime crash\n"
            f"# where: {where}\n"
