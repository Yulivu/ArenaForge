...
The intake REPL (``arenaforge-runtime`` with no subcommand) is a planning chat held *before*
...
runs persist under ``.arenaforge/runtime/sessions/<run_name>/`` (see
:mod:`arenaforge-runtime.coordinator.checkpoint`); this module is the analogous, deliberately
lighter store for the *conversation* itself.

Layout, one dir per conversation under the launch directory::

    <cwd>/.arenaforge/runtime/conversations/<conv_id>/
...
(:func:`arenaforge-runtime.coordinator.checkpoint.write_messages` /
:func:`~arenaforge-runtime.coordinator.checkpoint.read_messages`) so a conversation and a run
...
def conversations_root(cwd: str | os.PathLike[str]) -> Path:
    """Return ``<cwd>/.arenaforge/runtime/conversations`` (the per-project conversation root)."""
...
def _messages_for_disk(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    user authorized ArenaForge Research Runtime to inspect.
    """
