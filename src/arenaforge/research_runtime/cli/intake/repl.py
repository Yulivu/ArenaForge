...
async def run_intake(
    *,
    ...
    ``<starting_cwd>/.arenaforge/runtime/conversations/`` so it can be continued later. With
    ``continue_latest`` (``arenaforge-runtime --continue``) the newest unfinished
...
def _print_llm_failure_hint(*, provider: str, model: str | None, base_url: str | None) -> None:
    ...
    lines = [
        "The planning model did not answer, so ArenaForge Research Runtime stayed in intake instead of launching a run.",
        "",
    ...
    lines.extend([
        "",
        "Check that the endpoint is running and serving that model, or update `arenaforge-runtime config init ... --force`.",
...
def _print_resumed_history(messages: list[dict]) -> None:
    ...
    for m in messages:
        ...
        if role == "user":
            ...
            _console.print(f"[cyan]arenaforge-runtime ›[/cyan] {escape(text)}")
    if rendered:
    ...
    *,
    ...
    if cmd == "/help":
        ...
        if not convs and not runs:
            ...
            _console.print(
                "  [dim]Conversations live in [/dim][cyan].arenaforge/runtime/conversations/[/cyan]"
                "[dim] and launched runs in [/dim][cyan].arenaforge/runtime/sessions/[/cyan][dim], "
