"""`arenaforge-runtime config` — view and edit user-level defaults."""
...
def init_command(
    ...
    Examples:
            ...
            # auto — let ArenaForge Research Runtime probe the endpoint and pick the best backend
            # (recommended for DeepSeek / Qwen / GLM and other OpenAI-compatible APIs)
            arenaforge-runtime config init --provider auto --model deepseek-reasoner \
                --base-url https://api.deepseek.com --api-key sk-...

            # OpenAI / o-series via the Responses API (reasoning chain preserved)
            arenaforge-runtime config init --provider openai-responses --model gpt-5.5 \
                --base-url http://localhost:4141 --api-key dummy

            # any OpenAI-compatible chat-completions endpoint
            arenaforge-runtime config init --provider openai-chat --model qwen3.7-max \
                --base-url https://dashscope.aliyuncs.com/compatible-mode/v1 --api-key sk-...

              # Claude via Anthropic with env-var ANTHROPIC_API_KEY
              arenaforge-runtime config init --provider anthropic --model claude-sonnet-4-20250514
    """
...
def write_user_llm_config(llm: dict[str, Any]) -> None:
    ...
    Shared by ``arenaforge-runtime config init`` and the interactive ``arenaforge-runtime setup`` wizard so
