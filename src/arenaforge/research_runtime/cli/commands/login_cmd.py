"""`arenaforge-runtime login` — subscription OAuth login (experimental).

Supports ChatGPT (OpenAI) and Claude (Anthropic) subscriptions: ``arenaforge-runtime login
openai`` / ``arenaforge-runtime login claude`` run a browser OAuth flow so subscribers can
drive ArenaForge Research Runtime with their subscription instead of a pay-per-token API key.
...
def login_openai(
    ...
    """Log in to ChatGPT and store the subscription token under ~/.arenaforge/runtime/oauth/."""
    ...
    _console.print("Run [bold]arenaforge-runtime[/] to start a session.")
...
def login_claude(
    ...
    """Log in to Claude and store the subscription token under ~/.arenaforge/runtime/oauth/."""
...
def login_status() -> None:
    ...
    if not any_signed_in:
        _console.print(
            "[yellow]not signed in[/] — run `arenaforge-runtime login openai` or `arenaforge-runtime login claude`"
        )
        raise typer.Exit(code=1)
