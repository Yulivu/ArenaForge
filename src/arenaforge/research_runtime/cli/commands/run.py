"""`arenaforge-runtime run` — chat with the intake agent, then run the coordinator."""
...
_AUTO_CONFIG_NAMES = ("research_config.yaml", "arenaforge.research_runtime.yaml", "autoresearch.yaml")
...
def run_command(
    ...
    config: Path | None = typer.Option(
        None, "--config", "-c",
        help="Project YAML config. Defaults to research_config.yaml / arenaforge-runtime.yaml / autoresearch.yaml in the target project.",
    ),
    ...
    run_name: str | None = typer.Option(None, "--run-name", help="Session name under .arenaforge/runtime/sessions/. Defaults to timestamp."),
    ...
    # user through `arenaforge-runtime setup` once instead of failing on missing credentials,
    ...
    # from arenaforge-runtime setup / project YAML; run-time CLI only overrides execution
    ...
    arenaforge_ledger = None
    arenaforge_ledger_path = os.environ.get("ARENAFORGE_LEDGER_PATH")
    if arenaforge_ledger_path:
        # Optional product hook. ArenaForge Research Runtime remains fully usable without ArenaForge;
        # the bridge is enabled only when a caller explicitly provides a ledger.
        try:
            from arenaforge.arbor_bridge import attach_ledger
            from arenaforge.evidence import EvidenceLedger

            arenaforge_ledger = EvidenceLedger(
                arenaforge_ledger_path,
                os.environ.get("ARENAFORGE_RUN_ID") or resolved_run_name,
            )
            attach_ledger(
                arenaforge_ledger,
                bus,
        ...
        except Exception as exc:  # pragma: no cover - provider runs are external
            render_status(
                f"failed to attach ArenaForge ledger ({exc!r})",
                style="yellow",
                glyph="!",
            )

    # The previous "arenaforge-runtime run" banner panel (task/cwd/model/cycles/...)
    ...
    if webui is not None:
        webui.stop()
    if arenaforge_ledger is not None and not arenaforge_ledger.verify():
        render_status(
            "ArenaForge ledger verification failed after ArenaForge Research Runtime session",
            style="yellow",
            glyph="!",
        )
...
def _add_effective_llm_overrides(overrides: dict[str, Any], eff: dict[str, Any]) -> None:
    """Copy LLM values resolved from arenaforge-runtime setup/project YAML into config overrides.

    ``arenaforge-runtime run`` no longer accepts model/provider flags, but the final
    CoordinatorConfig must still receive the values selected by ``arenaforge-runtime setup``
