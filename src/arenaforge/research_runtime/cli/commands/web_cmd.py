"""``arenaforge-runtime web`` — open a read-only browser monitor for a session.

Unlike the live dashboard (which only exists during ``arenaforge-runtime run``), this serves
a session *from disk*, so it works for the keyless, host-driven flow: while a
coding agent drives an ArenaForge Research Runtime run via the ``arenaforge-runtime mcp`` tools, the user (or the
...
def web_command(
    session: str = typer.Argument(
        ..., help="Session name or path (under .arenaforge/runtime/sessions/), e.g. the run name."
    ),
    cwd: Path | None = typer.Option(
        None, "--cwd", help="Project directory holding .arenaforge/runtime/sessions/ (default: current directory)."
    ),
    ...
    """Serve a read-only web monitor for an ArenaForge Research Runtime session and block until Ctrl-C."""
