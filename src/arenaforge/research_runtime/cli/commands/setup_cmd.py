"""`arenaforge-runtime setup` — interactive one-time configuration wizard.

A first-time user runs ``arenaforge-runtime`` (or ``arenaforge-runtime setup``) and answers a few prompts;
we write ``~/.arenaforge/runtime/config.yaml`` so subsequent runs need no flags. The wizard is
the interactive sibling of the flag-driven ``arenaforge-runtime config init`` and shares its
...
def run_setup_wizard(*, force: bool = False) -> bool:
    ...
    :func:`write_user_llm_config` so the file matches ``arenaforge-runtime config init``.
    """
    ...
    if GLOBAL_CONFIG_FILE.exists() and not force:
        ...
        if not typer.confirm("Overwrite it?", default=False):
            _console.print("[dim]Keeping the existing config. Run `arenaforge-runtime config show` to view it.[/]")
            return False

    _console.print()
    _console.print("[bold cyan]arenaforge-runtime setup[/] — let's configure your model (one time).")
    ...
    if easy_llm is not None:
        ...
        _console.print(
            ...
            "([dim]view it anytime with[/] [bold]arenaforge-runtime config show[/])."
        )
        _console.print("Just run [bold]arenaforge-runtime[/] to start a session.\n")
        return True
    ...
    provider = _select_choice(
        "API type",
        options=[
            ("auto", "let ArenaForge Research Runtime detect it — probes Responses API, else chat completions"),
...
def _choose_easy_start() -> dict[str, str] | None:
    ...
    _console.print(
        "[dim]note: free-tier models are great for trying ArenaForge Research Runtime on your own task; "
...
def _setup_openai_oauth() -> bool:
    ...
    _console.print(
        f"\n[green]Done.[/] Saved to [bold]{GLOBAL_CONFIG_FILE}[/]. "
        "Just run [bold]arenaforge-runtime[/] to start a session.\n"
    )
    return True
...
def setup_command(
    ...
    """Interactively configure arenaforge-runtime (writes ~/.arenaforge/runtime/config.yaml)."""
...
def quickstart_command(
    ...
    Same writer as ``arenaforge-runtime setup``; this entry point just leads with the
