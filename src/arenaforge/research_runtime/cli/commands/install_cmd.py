"""``arenaforge-runtime install`` / ``arenaforge-runtime uninstall`` — manage the bundled Agent Skill suite.

ArenaForge Research Runtime ships an 11-directory *Agent Skill suite* (``skills/arenaforge-runtime-*``) that lets a
host coding agent (Claude Code, Codex, …) run the ArenaForge Research Runtime research workflow using
*its own* model — no ArenaForge Research Runtime API key, no separate runtime. Historically users had
to copy those directories by hand (see ``skills/README.md``). These commands
automate that: ``arenaforge-runtime install`` discovers the suite bundled with the installed
package and copies it into the right harness skills directory; ``arenaforge-runtime
uninstall`` removes exactly the directories ArenaForge Research Runtime owns and nothing else.

Design notes
------------
* **Idempotent.** Re-running ``install`` refreshes each ``arenaforge-runtime-*`` directory in
...
* **Namespaced + safe.** We only ever touch directories whose names start with
  ``arenaforge-runtime-``. A user's own unrelated skills living in the same target directory
  are never read, copied, or removed.
* **No LLM, no key.** This command performs pure filesystem work; it is part of
  ArenaForge Research Runtime's keyless coding-agent integration path.
...
SKILL_PREFIX = "arenaforge-runtime-"
...
def bundled_skills_root() -> Path:
    """Locate the ``skills/`` suite that ships with this ArenaForge Research Runtime install.

    Resolution order (first hit wins):

    1. ``$ARBOR_SKILLS_DIR`` — explicit override (tests / packaging debug).
    2. ``<arenaforge-runtime package>/skills_suite`` — the location the wheel ships the suite
       to (see ``pyproject.toml`` ``package-dir`` mapping).
    3. ``<repo root>/skills`` — an editable / source checkout, where the suite
       lives one level above the ``arenaforge-runtime`` package (``src/``).
    ...
    import arenaforge.research_runtime as research_runtime

    pkg_dir = Path(research_runtime.__file__).resolve().parent
    ...
    raise FileNotFoundError(
        "Could not locate the bundled ArenaForge Research Runtime skill suite. Looked under "
        f"${SKILLS_DIR_ENV}, {packaged}, and {dev}."
    )


def discover_skill_dirs(root: Path) -> list[Path]:
    """Return the ``arenaforge-runtime-*`` skill directories directly under *root*, sorted.
...
def install_skills(src_root: Path, dest: Path) -> list[str]:
    """Copy every ``arenaforge-runtime-*`` skill dir from *src_root* into *dest*.

    Existing ``arenaforge-runtime-*`` directories at the destination are removed first so the
...
def uninstall_skills(dest: Path) -> list[str]:
    """Remove only the ``arenaforge-runtime-*`` skill dirs from *dest*; leave everything else.
...
def install_command(
    ...
    """Install the bundled ArenaForge Research Runtime skill suite into a coding-agent skills directory."""
    ...
    for name in installed:
        typer.echo(f"  ✓ {name}")
    typer.secho(
        "\nRestart your coding agent, then run `/arenaforge-runtime-research-agent <your task>`.",
...
def uninstall_command(
    ...
    """Remove the ArenaForge Research Runtime skill suite from a coding-agent skills directory."""
    dest = resolve_target(claude=claude, codex=codex, project=project, target=target)
