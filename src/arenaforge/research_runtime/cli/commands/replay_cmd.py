"""`arenaforge-runtime replay` — replay a recorded session through the live dashboard.

Zero-setup way to *see ArenaForge Research Runtime work*: the live agent needs an LLM key, but
replaying a recorded run needs nothing. ``arenaforge-runtime replay --demo`` plays a bundled
sample; ``arenaforge-runtime replay <session>`` plays any real run's ``events.jsonl``.
"""
...
def replay_command(
    ...
    out: Optional[Path] = typer.Option(
        None, "--out", "-o", help="Output path for --html (default: <session>/arenaforge-runtime-tree.html).",
    ),
    ...
    """Replay a previous ArenaForge Research Runtime session — watch the idea tree grow with zero setup."""
...
def _resolve_source(source: Path, cwd: Path) -> Path:
    ...
    conventional sessions directory so ``arenaforge-runtime replay my-run`` works the same way
    ``arenaforge-runtime export my-run`` does.
    """
