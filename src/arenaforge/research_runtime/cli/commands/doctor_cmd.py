"""`arenaforge-runtime doctor` — diagnose install + runtime environment."""
...
def doctor_command() -> None:
    ...
    #     `arenaforge-runtime` from a fresh prompt. If these disagree, there's a
    #     shadowing arenaforge-runtime higher on PATH and the user is likely confused.
    ...
    if not running and not path_first:
        _fail(f"`{APP_NAME}` not on PATH",
              "pipx install -e /path/to/arenaforge-runtime")
        problems += 1
    else:
        if running:
    ...
    try:
        ...
        _ok(f"research runtime imports from {Path(research_runtime.__file__).parent}")
    except Exception as e:
        _fail(f"cannot import research runtime: {e!r}")
        problems += 1
