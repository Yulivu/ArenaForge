"""Top-level Typer app for the arenaforge-runtime CLI."""
...
# handling for `arenaforge-runtime --help`. Instead, we detect the no-subcommand case
...
def version_command() -> None:
    ...
    # The installed distribution is "arenaforge-runtime-agent"; APP_NAME ("arenaforge-runtime") is the
    # command/brand, not the package name, so look up the dist explicitly (with
    # a fallback in case it's ever renamed back).
    ver = "unknown"
    for dist in ("arenaforge-runtime-agent", APP_NAME):
        try:
            ver = _v(dist)
            break
        except Exception:
...
def main() -> None:
    ...
    If invoked with no subcommand (e.g. `arenaforge-runtime` or `arenaforge-runtime --cwd .`),
