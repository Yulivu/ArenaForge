...
needs nothing. ``arenaforge-runtime replay --demo`` is the zero-setup "see it work" path — the
...
def resolve_events_path(source: Path) -> Path:
    ...
    if source.is_dir():
        ...
        raise FileNotFoundError(
            f"no events.jsonl in {source} — is this an arenaforge-runtime session directory?"
        )
...
def print_recording_banner(rec: Recording, *, is_demo: bool) -> None:
    ...
    console.print(
        f"[bold yellow]arenaforge-runtime replay[/] [dim]·[/] {label} "
        f"[dim]·[/] {rec.event_count} events [dim]·[/] {dur_text}"
    )
    if is_demo:
        console.print(
            "[dim]illustrative sample — replays the dashboard with no API key. "
            "Run a real session with [/][cyan]arenaforge-runtime[/][dim] (needs a model).[/]"
        )
