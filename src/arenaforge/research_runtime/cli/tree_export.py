...
event bus — in two surfaces. ``arenaforge-runtime replay`` paints the tree in your terminal;
``arenaforge-runtime replay --html`` writes a single self-contained file that animates the
...
def default_html_path(rec: Recording) -> Path:
    ...
    if rec.session_dir is not None:
        return Path(rec.session_dir) / "arenaforge-runtime-tree.html"
    return Path.cwd() / f"arenaforge-runtime-tree-{rec.run_name}.html"
