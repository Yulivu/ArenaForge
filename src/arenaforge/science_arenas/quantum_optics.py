"""Quantum-optics evaluation bridge for the GOAI open-exploration demo.

PyTheus supplies the quantum experiment graph model and fidelity/count-rate
objectives. ArenaForge adds a declared transmission-loss sweep so candidates
are compared on robustness rather than on a single idealized score.
"""
...
def _load_pytheus(pytheus_root: str | Path | None) -> tuple[Any, Any, Any, Any]:
    ...
    except ImportError as exc:  # pragma: no cover - depends on optional arena setup
        raise RuntimeError(
            "PyTheus is required for the quantum-optics arena. "
            "Set PYTHEUS_ROOT or install the bundled third_party/pytheus package."
        ) from exc
...
def _weight_for_loss(edge: tuple[int, int, int, int], eta: float, detectors: set[int]) -> float:
    """Apply a transparent per-edge transmission proxy.

    The demo models a path touching an ancillary detector as one additional
    lossy segment. This is intentionally a declared proxy, not a claim that it
    replaces a laboratory optical calibration.
    """
