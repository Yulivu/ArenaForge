...
本次 replay 使用真实 PyTheus 评估器完成边际影响筛查、反馈引导删边、阈值参照和
随机负对照的统一运行：

| 分支 | 连接数 | robust score | 最大质量下降 | 结论 |
|---|---:|---:|---:|---|
| canonical PyTheus | `74` | `0.763186` | `0%` | 性能参照，但超出预算 |
| sensitivity-guided `025` | `49` | `0.739678` | `1.92%` | 74 次筛查、25 次接受行动后推荐 |
| threshold `0.150` | `49` | `0.739678` | `1.92%` | 启发式策略参照 |
| threshold `0.120` | `51` | `0.748234` | `1.73%` | 满足协议 |
| guided boundary action | `48` | `0.732123` | `2.32%` | 第 26 次行动触发质量边界 |
...
可行拓扑。未参与搜索的 transmission `0.98, 0.85, 0.75` 上，最大质量下降为
`1.80%`，独立验证通过。
